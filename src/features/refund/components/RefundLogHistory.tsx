...
import { MantineReactTable, MRT_FilterOption, useMantineReactTable } from 'mantine-react-table';
import { MRT_Localization_ES } from 'mantine-react-table/locales/es/index.cjs';
import { ActionIcon, Button, Tooltip } from '@mantine/core';
import { REFUND_LOG_COLUMNS } from '../const';
import { refundQueries } from '../query';
import { useTable } from '~/features/core/hooks/useTable';
...
export const RefundLogHistory = () => {
  const { table } = useTable({
    ...
    // TODO verify this type in queryOptions tanstack query
    getData: refundQueries.logs,
  });
  // const columns = useMemo(() => REFUND_LOG_COLUMNS, []);

  // const columnsFilter = Object.fromEntries(
  //   columns.map(({ accessorKey }) => [accessorKey, 'contains'])
  // );
  // // const routeApi = getRouteApi(RefundRoute.id);
